...
//    Some(Id(5)).optic.composePrism(Prism.some).composeIso(id).getOption shouldEqual Some(5)
    ...
    // here is an example where variance in optics would help
...
//    PPrism.some.andThenIso(id).getOption(Some(Id(5))) shouldEqual Some(5)
    ...
    PPrism.some[Id[Int], Id[String]].andThenIso(id).set("foo")(Some(Id(5))) shouldEqual Some(Id("foo"))
...
//    Some(Id(5)).optic.andThenPrism(poly.PPrism.some).andThenIso(id).getOption shouldEqual Some(5)
    ...
    // type mismatch;
    //[error]  found   : monocle.poly.PPrism[Option[Id[Int]],Option[Nothing],Id[Int],Nothing]
    //[error]  required: monocle.poly.PPrism[Some[Id[Int]],Option[Id[String]],Id[Int],C2]
//    Some(Id(5)).optic[Option[Id[String]]].andThenPrism(poly.PPrism.some).andThenIso(id).set("foo") shouldEqual Some(Id("foo"))

    Some(Id(5)).optic.andThenPrism(poly.Prism.some).andThenIso(id).getOption shouldEqual Some(5)
    Some(Id(5)).optic.andThenPrism(poly.Prism.some).andThenIso(id).set(4) shouldEqual Some(Id(4))
